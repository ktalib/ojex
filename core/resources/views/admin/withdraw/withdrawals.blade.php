                            ...
                            <tbody>
                                @forelse($withdrawals as $withdraw)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {{ showAmount($withdraw->amount, 2) }} - <span class="text--danger" title="@lang('charge')">{{ showAmount($withdraw->charge, 2) }} </span>
                                            <br>
                                            <strong title="@lang('Amount after charge')">
                                                {{ showAmount($withdraw->amount - $withdraw->charge, 2) }}
                                            </strong>
                                        </td>
                                        <td>
                                            {{ showAmount(1, 2) }} = {{ showAmount($withdraw->rate, 2, currencyFormat: false) }} {{ __($withdraw->currency) }}
                                            <br>
                                            <strong>{{ showAmount($withdraw->final_amount, 2, currencyFormat: false) }} {{ __($withdraw->currency) }}</strong>
                                        </td>
